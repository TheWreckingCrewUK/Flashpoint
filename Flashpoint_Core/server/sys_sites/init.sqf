...
[twc_capitalLocation,twc_capitalFlag,twc_CityTransportDistance,100] call twc_fnc_initSite;

nameCityLocationArray = nearestLocations [[worldSize / 2, worldSize / 2], ["NameCity"], (sqrt 2 *(worldSize / 2))];
{
	[_x,twc_cityFlag,twc_townTransportDistance,30] call twc_fnc_initSite;
/*	
...
{
	[_x,twc_villageFlag,twc_villageTransportDistance,10] call twc_fnc_initSite;
}forEach nameVillageLocationArray;

twc_missionInitComplete = true;
/*
nameLocalLocationArray = nearestLocations [[worldSize / 2, worldSize / 2], ["nameLocal"], (sqrt 2 *(worldSize / 2))];
{
	if(count (nearestObjects [getPos _x,twc_radioTowers,200]) > 0)then{
		...
		[_x,twc_localFlag,"placeHolder",1] call twc_fnc_initSite;
	};
...
_radio = nearestObjects [[worldSize / 2, worldSize / 2],twc_radioTowers,(sqrt 2 *(worldSize / 2))];
{
	...
	_loc setText "Radio";
	[_loc,twc_radioFlag,"placeHolder",1] call twc_fnc_initSite;
}forEach _radio;
...
_power = nearestObjects [[worldSize / 2, worldSize / 2],twc_powerPlants,(sqrt 2 *(worldSize / 2))];
{
	...
	_loc setText "Power";
	[_loc,twc_powerFlag,"placeHolder",1] call twc_fnc_initSite;
}forEach _power;

nameAirportLocationArray = nearestLocations [[worldSize / 2, worldSize / 2], ["Airport"], (sqrt 2 *(worldSize / 2))];
{
	[_x,twc_airportFlag,"placeHolder",1] call twc_fnc_initSite;
...
townLocationArray = nearestLocations [[worldSize / 2, worldSize / 2], ["NameVillage","NameCity","NameCityCapital","nameLocal"], (sqrt 2 *(worldSize / 2))];
{
	call{
		...
		if(text _x in _installationNames)exitWith{
			[_x,_installationMarker,"placeHolder",1] call twc_fnc_initSite;
		};
		if(text _x in _smallTownNames)exitWith{
			[_x,_smallTownMarker,"placeHolder",1] call twc_fnc_initSite;
		};
		if(text _x in _navalNames)exitWith{
			[_x,_navalMarker,"placeHolder",1] call twc_fnc_initSite;
		};
		if(text _x in _airfieldNames)exitWith{
			[_x,_airfieldMarker,"placeHolder",1] call twc_fnc_initSite;
		};
		if(text _x in _militaryNames)exitWith{
			[_x,_militaryMarker,"placeHolder",1] call twc_fnc_initSite;
		};		
		//[_x,_infMarker,"placeHolder",1] call twc_fnc_initSite;
	};	
